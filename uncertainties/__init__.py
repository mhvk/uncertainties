...
try:
    ...
    FLOAT_LIKE_TYPES += (np.number, np.ndarray)
    CONSTANT_TYPES += FLOAT_LIKE_TYPES[-2:]
    ...
    def correlated_values(nom_values, covariance_mat, tags=None):
        ...
        (variances, transform) = np.linalg.eigh(covariance_mat)
    ...
    def correlated_values_norm(values_with_std_dev, correlation_mat,
        ...
        return correlated_values(
            nominal_values,
            correlation_mat*std_devs*std_devs[np.newaxis].T,
            tags)
...
class AffineScalarFunc(object):
    ...
    def __init__(self, nominal_value, derivatives):
        ...
        #MHvK self._nominal_value = float(nominal_value)
    ...
    # not always yield a boolean (for instance, 0 <= np.arange(10)
    # yields an array).
    ...
    def error_components(self):
        ...
        for (variable, derivative) in self.derivatives.iteritems():
            ...
            if np.any(np.isnan(derivative)):
                error_components[np.logical_and(np.isnan(derivative),
                                                variable._std_dev == 0.)] = 0.
    ...
    def std_dev(self):
        ...
        return np.sqrt(sum([delta**2 for delta in
                            self.error_components().itervalues()]))
    ...
    for (op, derivative) in (
        ...
        try:
            setattr(AffineScalarFunc, attribute_name,
                    wrap(getattr(np.ndarray, attribute_name), [derivative]))
        except AttributeError:
            pass
        else:
    ...
    for (op, derivatives) in ops_with_reflection.iteritems():
        ...
        try:
            if op not in custom_ops:
                func_to_wrap = getattr(np.ndarray, attribute_name)
            else:
...
try:
    ...
    def correlation_matrix(nums_with_uncert):
        ...
        cov_mat = np.array(covariance_matrix(nums_with_uncert))
